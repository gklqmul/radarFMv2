...
class SingleFrameFlowTransformer(nn.Module):
    ...
    def __init__(self, num_joints=27, radar_in_channels=6, embed_dim=512,
                 local_radius=0.1, parent_list=None, nhead=8, num_layers=6):
        super().__init__()
        self.num_joints = num_joints
    ...
    def forward(self, x_t, t, x0, pc_raw):
        ...
        h_doppler = self.doppler_head(pc_doppler).mean(dim=1, keepdim=True)  # [BT,1,D]
...
class RadarPoseRefiner(nn.Module):
    ...
    """
    def __init__(self, in_channels=4, radar_embed_dim=256, num_latents=64,
    ...
    def forward(self, x_t, t, radar_input):
        ...
        radar_raw = radar_input.flatten(0, 1)  # [BT,N,4]
    ...
    @torch.no_grad()
    def inference(self, radar_input, steps=8, method="heun"):
        """
        多步 ODE 推理：
...
def evaluate_sequence(
    ...
    for batch_idx, (radar_seq, skeleton_seq) in enumerate(pbar):
        ...
        # 2) Inference
        # -----------------------
        pred = model.inference(radar_input, steps=steps)  # [B,T,J,3] meters
...
def main():
    ...
    # 设置保存路径
    save_dir = "/code/radarFMv2/checkpoints_baseline11"
    ...
    # ckpt_stage1 = "/code/radarFMv2/checkpoints_baseline10/best_model.pt"
    # model.load_pretrained_stage1(ckpt_stage1, freeze=True)

    ckpt_path = "/code/radarFMv2/checkpoints_baseline11/best_refiner.pt"
    ...
    #             loss = model.compute_fm_loss(radar_input, skeleton_seq)
