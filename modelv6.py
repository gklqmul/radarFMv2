...
class FiLMLayer(nn.Module):
    ...
    def forward(self, x, condition):
        # condition 可以是 h_tau 或 [h_tau + global_z]
        res = self.mlp(condition).unsqueeze(1) # [B, 1, 2*D]
        scale, shift = torch.chunk(res, 2, dim=-1)
...
def GFM_skeleton_mask(parent_list, num_joints=27):
    ...
    return mask # 用于 transformer 的 tgt_mask 参数
...
class SingleFrameFlowTransformer(nn.Module):
    ...
    def __init__(self, num_joints=27, radar_embed_dim=256, embed_dim=512):
        ...
        # 【新增】残差流向 Embedding: 专门处理 (x_t - x_coarse)
        self.diff_embed = nn.Linear(3, embed_dim)
        
        # 【新增】关节 ID 编码: 让模型识别出“末端”与“躯干”
        ...
        # 记录末端点索引 (用于 Loss 加权或调试)
        self.end_effectors = [9, 10, 16, 17, 21, 25, 26]

    def forward(self, x_t, tau, z_radar, x_coarse, parent_list):
        B, N, _ = x_t.shape
        ...
        # 构造 ID 编码 [1, 27, 512]
        joint_ids = torch.arange(N, device=x_t.device)
        h_id = self.joint_id_embed(joint_ids).unsqueeze(0) 
        ...
        # 计算流向残差并 Embedding
        diff = x_t - x_coarse
        h_diff = self.diff_embed(diff) 
        
        # 2. 引入层级扩散 (GFM 核心)
        # parent_list 转换为 tensor 索引
        p_idx = torch.tensor(parent_list, device=x_t.device).long()
        h_parent = h_xt[:, p_idx, :] 
        
        # 聚合邻居特征
        h_struct = self.diffusion_proj(torch.cat([h_xt, h_parent], dim=-1))
        
        # 3. 构造 Query：将 语义、几何、时间、动力学 四者合一
        # 这里的相加操作要求所有项的 shape 都是 [B, 27, 512]
        query = h_struct + h_tau + h_coarse + h_diff + h_id
        
        # 4. 执行 Transformer 交互
        refined_feat = self.transformer(tgt=query, memory=z_radar) 
        
        # 5. 预测速度场
        vt = self.vel_head(refined_feat)
        return vt
...
class RadarPoseRefiner(nn.Module):
    ...
    def forward(self, x_t, t, radar_input):
        ...
        v_pred = self.refiner(x_t, t, z_seq, x_coarse, PARENT_LIST)
        return v_pred
    ...
    def compute_fm_loss(self, radar_input, x_gt):
        ...
        # 1. 强化末端点权重 (建议比之前更高，压制边缘误差)
        weights = torch.ones(27, device=device)
        end_indices = [9, 10, 16, 17, 21, 25, 26]
        weights[end_indices] = 4.0  # 提高到 4.0，强制模型修边缘
        w_expanded = weights.view(1, 27, 1)
    
        # 2. 获取 Stage 1 输出 (不计梯度)
        with torch.no_grad():
            z_seq, x_coarse = self.get_coarse_prior(radar_input)
        ...
        # 3. 采样 t 并构造 Path
        t = torch.rand(B * T, 1, device=device) * 0.999 
        ...
        # --- 核心改进 A: 基础速度 MSE + 时间加权 ---
        # t 越大，代表扰动越小，模型应该预测得越精准
        time_weight = t_broad + 0.5 # [0.5, 1.5] 的线性加权
        loss_mse = (w_expanded * time_weight * (v_pred - v_target) ** 2).mean()
    
        # --- 核心改进 B: 方向余弦损失 (解决 Step 依赖问题) ---
        # 强制路径变直，让 Step=10 也能达到 Step=50 的效果
        unit_v_pred = F.normalize(v_pred, dim=-1, eps=1e-8)
        unit_v_target = F.normalize(v_target, dim=-1, eps=1e-8)
        # 1 - cos_sim: 0 为方向完全一致
        loss_direction = (w_expanded.squeeze(-1) * (1.0 - torch.sum(unit_v_pred * unit_v_target, dim=-1))).mean()
    
        # --- 核心改进 C: 几何约束优化 ---
        # 依然保留对终点的预测，但增加对预测点 x1_pred 的长度惩罚
        x1_pred = x_t + v_pred * (1.0 - t_broad)
        
        gt_lens, gt_dirs = self.calc_bone_lengths_and_dirs(x_gt_flat, self.parent_dict)
        pred_lens, pred_dirs = self.calc_bone_lengths_and_dirs(x1_pred, self.parent_dict)
        
        # 骨骼长度误差：使用 L1 Loss 对长度更敏感，防止平均化导致的骨骼缩水
        loss_bone_len = F.l1_loss(pred_lens, gt_lens) 
        # 骨骼方向误差：确保生成的姿态“看起来”是对的
        loss_bone_dir = F.mse_loss(pred_dirs, gt_dirs)
    
        # 5. 组合总 Loss
        # 建议比例：MSE(1.0) + 方向(1.0) + 长度(0.5) + 几何方向(0.1)
        total_loss = loss_mse + 1.0 * loss_direction + 0.5 * loss_bone_len + 0.1 * loss_bone_dir
    ...
    def inference(self, radar_input, steps=10):
        """
        基于 ODE 积分的推理过程。
        随着 steps 增加，结果应该由于修正后的训练逻辑而变得更精确。
        """
        self.eval()
        B, T = radar_input.shape[:2]
        device = radar_input.device
        ...
        for i in range(steps):
            ...
            # 预测当前位置的速度向量
            v_pred = self.refiner(x_curr, t_tensor, z_seq, x_coarse, PARENT_LIST)
...
@torch.no_grad()
def diagnose_flow_smoothness(model, radar_input, steps=50, joint_idx=10, save_path="/code/radarFMv2/flow_diagnosis.png"):
    """
    诊断流场平滑度
    joint_idx: 观察的关节索引（建议选边缘点，如手指或足尖）
    """
    model.eval()
    B, T = radar_input.shape[:2]
    device = radar_input.device
    ...
    # 1. 获取起点
    z_seq, x_coarse = model.get_coarse_prior(radar_input)
    x_curr = x_coarse.clone()
    dt = 1.0 / steps
    
    trajectories = [] # 记录每一阶段的坐标
    velocities = []   # 记录每一阶段预测的速度向量
    ...
    for i in range(steps):
        ...
        # 记录当前位置
        trajectories.append(x_curr.cpu().clone())
        
        # 预测速度
        v_pred = model.refiner(x_curr, t_tensor, z_seq, x_coarse, PARENT_LIST)
        velocities.append(v_pred.cpu().clone())
        
        # Euler 步进
        x_curr = x_curr + v_pred * dt

    # 3. 计算余弦相似度 (相邻步之间的方向一致性)
    cos_sims = []
    for i in range(len(velocities) - 1):
        v1 = velocities[i][:, joint_idx, :]
        v2 = velocities[i+1][:, joint_idx, :]
        sim = F.cosine_similarity(v1, v2, dim=-1).mean().item()
        cos_sims.append(sim)
    ...
    traj_pts = torch.stack(trajectories)[:, 0, joint_idx, :].numpy() # 取 Batch 0
    ax1.plot(traj_pts[:, 0], traj_pts[:, 1], traj_pts[:, 2], 'b-o', markersize=2, label='Step Path')
    ax1.scatter(traj_pts[0, 0], traj_pts[0, 1], traj_pts[0, 2], color='red', s=50, label='Start (t=0)')
    ax1.scatter(traj_pts[-1, 0], traj_pts[-1, 1], traj_pts[-1, 2], color='green', s=50, label='End (t=1)')
    ax1.set_title(f'Joint {joint_idx} Trajectory')
    ax1.legend()

    # 子图2: 余弦相似度曲线
    ax2 = fig.add_subplot(132)
    ax2.plot(range(len(cos_sims)), cos_sims, 'r-')
    ax2.set_ylim(-1, 1.1)
    ax2.set_xlabel('Step')
    ax2.set_ylabel('Cosine Similarity with Next Step')
    ax2.set_title('Directional Consistency')
    ax2.grid(True)

    # 子图3: 速度模长变化 (Velocity Norm)
    ax3 = fig.add_subplot(133)
    v_norms = [torch.norm(v[0, joint_idx, :]).item() for v in velocities]
    ax3.plot(range(len(v_norms)), v_norms, 'g-')
    ax3.set_xlabel('Step')
    ...
    plt.tight_layout()
    plt.savefig(save_path, dpi=200) # 保存到服务器当前目录
    plt.close() # 及时释放内存
...
def main():
    ...
    is_main = (rank == 0)
    if is_main:
        print(f"DDP: {ddp} | world_size={world_size} | rank={rank} | local_rank={local_rank} | device={device}")
    ...
    # 2) 路径与缓存
    # ----------------------------
    save_path = "/code/radarFMv2/checkpoints_refinev3"
    if is_main:
    ...
    # 注意：使用 sampler 时 DataLoader 的 shuffle 必须 False
    train_loader = DataLoader(
        train_set,
        batch_size=4,  # 这是“每张 GPU 的 batch”，总 batch = 16 * world_size
    ...
    # 验证：为了省事、避免做跨进程指标汇总，推荐只在 rank0 跑完整 val
    # 其他 rank 不跑 val，直接 barrier 同步即可
    if is_main:
    ...
    # ckpt_path = "/code/radarFMv2/checkpoints_refinev3/best_refiner.pt"
    ...
    # radar_input, skeleton_input = batch # 根据你的数据结构调整
    # radar_input = radar_input.to(device)
    
    # # 2. 调用诊断函数
    # # 建议观察末端关节，例如手指（索引10）或足尖（索引21）
    # # 因为这些点的误差最能反映流场的不稳定性
    # print("正在诊断流场平滑度...")
    # cos_sim_list = diagnose_flow_smoothness(
    #     model=model, 
    #     radar_input=radar_input, 
    #     steps=9,      # 步数设多一点，容易看清轨迹
    #     joint_idx=10   # 观察右手腕/手尖
    # )
    
    # # 3. 分析输出
    # avg_sim = sum(cos_sim_list) / len(cos_sim_list)
    # print(f"平均方向一致性 (Cosine Similarity): {avg_sim:.4f}")
    ...
    if ddp:
        # find_unused_parameters=False 通常更快；如果你发现某些参数没梯度报错，再改 True
        model = DDP(model, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)

    # ----------------------------
    # 5) 优化器 / scheduler / AMP
    # ----------------------------
    # 只更新 requires_grad=True 的参数（refiner）
    ...
    for epoch in range(1, 301):
        ...
        # DDP：每个 epoch 需要 set_epoch 来打乱数据
        if ddp:
        ...
        # 只让 rank0 显示 tqdm，其他 rank 安静跑
        pbar = tqdm(train_loader, desc=f"Epoch {epoch} (rank {rank})", disable=not is_main)
        ...
        for radar_seq, skeleton_seq in pbar:
            ...
            if use_cuda:
                with torch.amp.autocast("cuda"):
                    # DDP 不会自动并行自定义方法，但我们是在每个进程本地跑自己的 batch，这就是 DDP 正常方式
                    loss = model.module.compute_fm_loss(radar_seq, skeleton_seq) if ddp else model.compute_fm_loss(radar_seq, skeleton_seq)
        ...
        # --- 训练 loss 做一个全局平均（可选，但更真实）---
        if ddp:
            # 把各 rank 的 loss_sum/step_sum 做 reduce
            loss_tensor = torch.tensor([total_epoch_loss, float(num_steps)], device=device)
            dist.all_reduce(loss_tensor, op=dist.ReduceOp.SUM)
            total_epoch_loss_all = loss_tensor[0].item()
            total_steps_all = max(int(loss_tensor[1].item()), 1)
            avg_train_loss = total_epoch_loss_all / total_steps_all
        else:
            avg_train_loss = total_epoch_loss / max(len(train_loader), 1)
        ...
        if (epoch % 2 == 0 or epoch == 1):
            if is_main:
                ...
                metrics = evaluate_sequence(
                    dataloader=val_loader,
                    model=core_model,     # 传 unwrap 后的模型，内部可直接用 model.inference/compute
                    device=device,
                    vis_dir=None,
                    vis_edges=EDGES_27,
                    num_vis_samples=1,
                ...
                # 兼容你旧版 evaluate_sequence 返回 float；新版返回 dict
                val_mpjpe = metrics["mpjpe"] if isinstance(metrics, dict) else metrics

                print(f"\n[Epoch {epoch}] Train Loss: {avg_train_loss:.6f} | Val MPJPE: {val_mpjpe:.2f}mm")
                ...
                if val_mpjpe < best_mpjpe:
                    ...
                    torch.save({
                        "epoch": epoch,
                        "model_state_dict": core_model.state_dict(),  # 保存“裸模型”权重（跨单卡/多卡通用）
                        "optimizer_state_dict": optimizer.state_dict(),
                        "best_mpjpe": best_mpjpe,
                    }, save_file)

                    print(f"⭐ New Best Model Saved: {val_mpjpe:.2f}mm -> {save_file}")

            # 同步一下，避免 rank0 在做 val 时其它 rank 进入下一轮产生干扰
            if ddp:
                dist.barrier()
