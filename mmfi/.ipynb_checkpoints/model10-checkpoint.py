...
class RadarPoseRefiner(nn.Module):
    ...
    def compute_fm_loss(self, radar_input, x_gt, noise_level=0.01, eps=1e-4):
        ...
        x1 = x_gt.reshape(B * T, self.num_joints, 3)
    ...
    def inference(self, radar_input, steps=20):
        ...
        for i in range(steps):
            ...
            v1 = self.refiner(x,t,coarse,radar_raw)
    
            x_temp = x + dt*v1

            t2 = torch.full((BT, 1), min((i + 1) / steps, 1.0), device=device)
            v2 = self.refiner(x_temp, t2, coarse, radar_raw)
...
def main():
    ...
    # 设置保存路径
    save_path = "/data/mmfi/checkpoints_basemodel10v2"
   ...
   # --- 2. 数据准备 ---
    ...
    ckpt_path = "/data/mmfi/checkpoints_basemodel10v2/best_refiner.pt"
