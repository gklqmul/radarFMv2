...
class DecoupledRegressionRefiner(nn.Module):
    ...
    """
    def __init__(self, num_joints=17, embed_dim=256):
        super().__init__()
        self.num_joints = num_joints
    ...
    def forward(self, x_coarse, z_radar_feat):
        ...
        x_feat = self.pos_embed(x_coarse)  # [BT, 17, 256]
        
        # --- 特征融合 (Memory) ---
        # 将 Coarse Pose 的位置特征和雷达特征拼接，作为 Cross-Attention 的“字典”
        # 维度：[BT, 17 + 64, 256] = [BT, 91, 256]
        memory = torch.cat([x_feat, z_radar_feat], dim=1) 
        
        # --- 采样特征 ---
        # Query: 预设的关节 Token [BT, 17, 256]
...
class RadarPoseRefiner(nn.Module):
    def __init__(self, in_channels=6, radar_embed_dim=256, num_joints=17, parent_list=None):
        super().__init__()
    ...
    def forward(self, radar_input):
        ...
        x_coarse, _, _ = self.coarse_head(z_global) # [BT, 17, 3]
    ...
    def compute_loss(self, radar_input, x_gt):
        ...
        x_pred = x_pred_full.view(B * T, 17, 3)
...
def main():
    ...
    save_path = "/data/mmfi/checkpoints_baseline6"
    ...
    # ckpt_stage1 = "/data/mmfi/checkpoints_baseline1/best_model.pt"
    # model.load_pretrained_stage1(ckpt_stage1, freeze=True)

    ckpt_path = "/data/mmfi/checkpoints_baseline6/best_refiner.pt"
    ...
    #         print(f"\n[Epoch {epoch}] Train Loss: {total_loss/len(train_loader):.6f} | Val MPJPE: {val_mpjpe:.2f}mm")
...
#     # 设置保存路径
#     save_path = "/data/mmfi/checkpoints_baseregression_crossenv"
#     os.makedirs(save_path, exist_ok=True)
...
#     ckpt_stage1 = "/data/mmfi/checkpoints_baseline1_crossenv/best_model.pt"
#     model.load_pretrained_stage1(ckpt_stage1, freeze=True)
