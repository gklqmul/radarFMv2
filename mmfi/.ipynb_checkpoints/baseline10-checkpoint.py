...
class RadarStage1Model(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert in_channels == 4, "Expect radar_cond dim=4"
...
def stage1_loss(pred, gt):
    """
    pred, gt: [B, T, 17, 3] (meters, root-relative)
    """
    valid_mask = (gt.abs().sum(dim=(2, 3)) > 1e-6)  # 过滤 Padding 帧

    err = torch.norm(pred - gt, dim=-1)             # L2 距离 [B, T, 17]
...
def main():
    ...
    save_dir = "/data/mmfi/checkpoints_baseline10"
    ...
    # ckpt_path = "/data/mmfi/checkpoints_baseline10/best_model.pt"
